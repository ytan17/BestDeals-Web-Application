	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		while(passedName.hasMoreElements()){
			...
			if(element!=null){
				if(element.contains("CheckOut")){
				...
				else if (element.contains("replacement1")){
					nameLst = element.split("_");
					for(String s: nameLst) {
						System.out.println(s);
					}
					...
					utility.setOIReplacement1(ioPrice*0.5, ioIndex);
				}
				else if (element.contains("replacement2")){
					nameLst = element.split("_");
					for(String s: nameLst) {
						System.out.println(s);
					}
					...
					utility.setOIReplacement2(ioPrice*0.65, ioIndex);
				}
...
/* displayCart Function shows the products that users has bought, these products will be displayed with Total Amount.*/
	...
	protected void displayCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (OrderItem oi : utility.getCustomerOrders()) 
		{
			...
			pw.print("<td>"+i+".</td><td>"+oi.getName()+"</td><td>: "+oi.getPrice()+"</td><td><input type='submit' name='Add"+add+"' value='Add' class='btnadd'></td><td><input type='submit' name='Delete_"+index+"' value='Delete' class='btndelete'></td><td><input type='submit' name='warranty_"+index+"' value='warranty:$50' class='btnwarranty'></td><td><input type='submit' name='replacement1_"+index+oi.getPrice()+"' value='1 year replacement' class='btnreplacement1'></td><td><input type='submit' name='replacement2_"+index+oi.getPrice()+"' value='lifetime replacement' class='btnreplacement2'>");
			...
			total = total +oi.getPrice()+oi.getWarranty()+oi.getReplacement1+oi.getReplacement2+oi.getShipping();
			i++;
		}
		...
		pw.print("</table>");
		pw.print("<br>")
		pw.print("<p>As Defual,Buy the new equipment with no replacement</p>");
		pw.print("<p>click on 1 year replacement, Buy the new equipment with 1 year replacement for 50% fee of the equipment retail price; under this plan the customer can replace the equipment by a new one any time during the year (Equipment can be replaced only once for the customer)</p>");
		pw.print("<p>click on lifetime replacement, Buy the new equipment with lifetime replacement for 65% fee of the equipment retail price; under this plan the customer can replace the equipment by another new one any time (Equipment can be replaced only once for the customer).</p>");
		}
		else
		{
